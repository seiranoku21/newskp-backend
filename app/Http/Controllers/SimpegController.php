    ...
    try {
        ...
        if (!empty($data)) {
            ...
            $data = array_map(function($item) use ($enrichSingle, $enrichLookup){
                ...
                $no_sk = null;
                $tgl_sk = null;
                if ($enrichSingle && $nipValue && isset($enrichLookup[$nipValue])) {
                    $e = $enrichLookup[$nipValue];
                    $pangkat_id = $e['pangkat_id'] ?? null;
                    $pangkat = $e['pangkat'] ?? null;
                    $kat_jabatan = $e['kat_jabatan'] ?? null;
...
function rmn_rwy_jabatan(Request $request){
    ...
    $mapRiwayatJabatan = static function (array $item) use ($normalizeDate, $resolveKodeJabatan) {
        ...
        return [
            ...
            'tst_sk' => $tglSelesai ?? '0000-00-00',
            'tgl_mulai' => $tglMulai ?? '0000-00-00',
            'tgl_selesai' => $tglSelesai ?? '0000-00-00',
            'tgl_sk' => $tglSk ?? '0000-00-00',
            ...
            if (isset($responseData['data']) && is_array($responseData['data'])) {
                ...
                $data[] = [
                    'id_riwayat_studi' => $item['no'] ?? null,
                        ...
                        'nm_prodi' => $item['jurusan'] ?? null,
                        'tgl_sk' => $item['tmt_mulai'] ?? null,
                        'tgl_masuk' => $item['tmt_mulai'] ?? null,
                        'tgl_lulus' => $item['tmt_akhir'] ?? null,
                        'tmt_sk' => $item['tmt_mulai'] ?? null,
                        'tst_sk' => $item['tmt_akhir'] ?? null,
                        'no_sk' => $item['no_sk'] ?? null,
                        'is_active' => $aktif_studi,
